...
var fluent = function (fields, starters) {
  ...
  fields = extend({}, defaultFields, fields || {});
    ...
    if (obj instanceof Array) {
      ...
      for (var j in obj) {
        ...
        if (Object.keys(obj[j]).length) {
          x = bindExtender(j, prepareFluentFields(obj[j]));
        } else {
          x = bindExtender(j);
        }
        Object.defineProperty(ret, j, {get: x, enumerable: true});
      }
    } else {
      throw new Error();
    }
  ...
  var appendGenerateFunction = function (obj, str) {
    ...
    return fluentReturn(obj, eval(fun), intern.fluentOperators, x); // eslint-disable-line no-eval
  };
